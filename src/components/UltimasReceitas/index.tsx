...
export default function UltimasReceitas({date, category=''}) {
    ...
    const loadData = async () => {
      try {
        ...
        const [authRes, lancamentosRes] = await Promise.all([
          ...
          api.get(`/expense/myExpenseByDate/${date}?startValue=0&category=${encodeURIComponent(category)}`, { headers }),
        ]);
...
/>
      <NormalText style={{marginBottom: "20"}}>Receitas { date}</NormalText>
