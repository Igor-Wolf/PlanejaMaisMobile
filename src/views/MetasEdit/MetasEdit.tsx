...
import { editGoalService } from "./actions";

export default function MetasEdit({ route }) {
  ...
  const [valueGoal, setValueGoal] = useState(
    String(Math.abs(item.goal).toFixed(2)),
  );
  ...
  const handleSave = async () => {
    try {
      ...
      try {
        const response = await editGoalService(item._id, updatedGoal);
        if (response.status === 200) {
          Alert.alert("Sucesso", "Meta atualizada com sucesso!");
          navigation.navigate("MinhasMetas");
        }else if (response.status === 409) {
                  Alert.alert("Erro de duplicidade", "Meta já criada no banco. É possivel criar apenas uma meta mensal ou anual");
                }
      } catch {}
    } catch (error) {
